 ...
 */
class BillDetailActivity : BaseActivity(), IViewBillDraw {
    ...
    override fun initData() {
        ...
        factory!!.setOnPrinterStateListener(object : ZBtConnFactory.OnPrinterStateListener {
            override fun stateOn() {
                imBlueTooth.setImageDrawable(ContextCompat.getDrawable(this@BillDetailActivity, R.mipmap.tms_icon_bluetooth_true))
                isConnSucc = true
            }

            override fun stateOff() {
                imBlueTooth.setImageDrawable(ContextCompat.getDrawable(this@BillDetailActivity, R.mipmap.tms_icon_bluetooth_false))
                isConnSucc = false
            }
        })
    ...
    private fun startBluePos(entity: BillingDrawMain) {
        ...
        override fun handleMessage(msg: Message?) {
            when (msg!!.what) {
                ...
                PRINTER_COMMAND_ERROR ->
                    Toasty.error(this@BillDetailActivity, "错误").show()

                CONN_PRINTER ->
                    Toasty.error(this@BillDetailActivity, "未连接").show()
    ...
    private fun setListeners() {
        tvPrint.setOnClickListener {
            if (ZShrPrefencs.getInstance().netOn == 0) {
                startPrint(detialEntity!!, ZPrintReceiver.REQUEST_BILL, 1, false)
            } else if (ZShrPrefencs.getInstance().netOn == 1) {
                print2CheckESC(detialEntity!!, ZPrintReceiver.REQUEST_BILL, 1, false)
            }
        }
        tvPrintBiao.setOnClickListener {
            if (ZShrPrefencs.getInstance().netOn == 0) {
                startBluePos(detialEntity!!)
            } else {
                print2CheckPOS(detialEntity!!, ZPrintReceiver.REQUEST_BILL, 1, false)
            }
        }
    }
