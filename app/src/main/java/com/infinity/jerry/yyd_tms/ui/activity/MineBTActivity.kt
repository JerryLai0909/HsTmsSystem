 ...
 */
class MineBTActivity : BaseActivity(), ZBtSetReceiver.OnBluetoothFoundListener {
    ...
    var name: String? = ZShrPrefencs.getInstance().blueToothDeviceName
    var mac: String? = ZShrPrefencs.getInstance().blueToothMac
...
//            showSaveNewBluetoothDialog(position)
//        }
    }
    ...
    private fun connBle(item: BluetoothDevice?) {
        ...
        if (mana != null) {
            if (mana!!.connState) {
                if (!item!!.address.equals(mac)) {
                    Toasty.info(this@MineBTActivity, "请先断开 " + name + " 的打印机").show()
                } else {
                    Toasty.info(this@MineBTActivity, "打印机连接成功").show()
                }
            } else {
                receiver!!.judgeTheName(item)
                name = item!!.name
                mac = item.address
                tvInfo.text = "当前连接的打印机: " + name + "  " + mac
            }
        ...
        ZShrPrefencs.getInstance().saveBlueToothDeviceName(item.name)
        ZShrPrefencs.getInstance().saveBlueToothMac(item.address)
        dialog!!.dismiss()
    }
        ...
        override fun handleMessage(msg: Message) {
            when (msg.what) {
                CONN_STATE_DISCONN -> if (DeviceConnFactoryManager.getDeviceConnFactoryManagers()[0] != null) {
                    ...
                    Toasty.info(this@MineBTActivity, "打印机已经断开").show()
                }
